...
const Nav = () => {
    ...
    return (
        <motion.nav initial={{ opacity: 0 }} animate={{ opacity: 100 }} transition={{ duration: 0.7 }} className='w-full h-auto lg:h-auto px-10 py-4 text-button z-50 top-0 left-0 absolute' style={active ? { position: "fixed" } : {}}>
            <div className="absolute w-full h-[120%] top-0 left-0 z-0 opacity-70 transition-all ease-in-out duration-300" style={scrolled ? {} : { opacity: "0%" }}></div>
            <div className="max-w-[1600px] w-full h-auto flex items-center justify-end mx-auto z-10 relative">
                ...
                <ul className={`items-center flex lg:relative fixed right-0 lg:w-auto lg:h-auto w-full h-full top-0 justify-center lg:justify-start flex-col lg:flex-row bg-[#111111] lg:bg-transparent gap-y-20 text-xl lg:text-sm opacity-0 transition-all ease-in-out duration-300 lg:opacity-100 lg:pointer-events-auto pointer-events-none font-saira ${scrolled ? "gap-x-12" : "gap-x-16"}`} style={active ? { opacity: "100%", pointerEvents: "auto" } : {}}>
                    ...
                    <a href="" target="_blank" rel="noreferrer noopener" className="">
                        <button className='w-fit h-auto px-10 py-3 font-aot bg-button text-white cursor-pointer hover:bg-buttonHover transition-all ease-in-out duration-300'>
                            <p>Launch DApp</p>
                        </button>
                    </a>
                </ul>
